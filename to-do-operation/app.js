...
function addToList() {
  ...
  if (term.trim()) {
    ...
    if (activeTodoID == -1) {
      ...
      let editedItem = titleList.find(
        (todoItem) => todoItem.workID == activeTodoID
      );
...
function renderToDoList() {
  ...
  addedList.innerHTML = `<ul>
                    ${titleList
                      .map((todoItem) => `${renderTodoItem(todoItem)}`)
                      .join("")}
                          </ul>`;
}
...
function toggleTodoItem(e) {
  ...
  let clickedOne = titleList.find(
    (todoItem) => todoItem.workID == successItemID
  );
