...
function onClickAddUser(event) {
  ...
  if (hasValid) {
    ...
    if (activeUserID == -1) {
      ...
      let editedItem = userDetailList.find((item) => item.ID == activeUserID);
      activeUserID = -1;
...
function renderUserList() {
  ...
  let userDetailList = getUserDetailList();
  tableBody[0].innerHTML = userDetailList
    .map((userItem) => {
      return `<tr>          
                <td>${userItem.ID}</td>
                ...
                <td><button><i class="fa fa-trash" data-delite-id="${userItem.ID}" onClick="onDeleteUserList(event)"></i></button></td>
                </tr>`;
    })
    .join(" ");
}
...
function onEditUserList(e) {
  ...
  let userSubmit = document.getElementById("userSubmit");
  userSubmit.textContent = "UPDATE";

  let userDomList = [username, email, address, city, contact];
  userDomList.forEach((domItem) => showSuccess(domItem));
}

function onDeleteUserList(e) {
