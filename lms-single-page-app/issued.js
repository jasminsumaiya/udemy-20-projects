...
function initIssuedPage() {
  ...
  let userOptionList = userDetailList.map(
    (userDetail) =>
      `<option value=${userDetail.ID}>${userDetail.USERNAME}</option>`
  );
  ...
  let bookOptionList = bookDetailList.map(
    (bookDetail) => `<option value=${bookDetail.ID}>${bookDetail.BOOK}</option>`
  );
...
function onIssueUserClick(e) {
  ...
  let selectedBookData = bookDetailList.find(
    (bookDEtail) => bookDEtail.ID == bookDropdown.value
  );
  console.log(selectedBookData);

  let givenCount = parseInt(countDom.value);
  let currentQuantity =
    parseInt(selectedBookData.QUANTITY) - parseInt(selectedBookData.ISSUEDBOOK);
  ...
  let newIssueBookCount =
    parseInt(selectedBookData.ISSUEDBOOK) + parseInt(countDom.value);
...
function checkRequired(domElList) {
  ...
  domElList.forEach((domItem) => {
    if (
      !domItem.value.trim() ||
      domItem.value.trim() == "User" ||
      domItem.value.trim() == "Book"
    ) {
      hasValid = false;
